// verify_jwt=false: every write instead requires a verified Google OIDC JWT.
import { createRemoteJWKSet, jwtVerify } from 'npm:jose@6.1.0';
import '../../vibe-arcade/games/orbit-sprint/core.js';
import '../../vibe-arcade/arcade3/core.js';
import '../verify-run.js';
const orbit=(globalThis as any).OrbitRules,arcade=(globalThis as any).ArcadeRules,verifyRun=(globalThis as any).LoopJoltVerify;
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  const bearer=req.headers.get('Authorization')||'';if(!bearer.startsWith('Bearer ')||bearer.length>8500)return reply({error:'authentication_required'},401);
  ...
  if(action==='finish'){
   ...
   let verified;try{verified=verifyRun(run,body,orbit,arcade);}catch(e){return reply({error:e instanceof Error&&e.message==='version_mismatch'?'version_mismatch':'invalid_game_replay'},400);}
