...
const C=window.LoopCommunity,GAMES=Object.freeze([
 ...
 {game:'gyro-drop',title:'Gyro Drop · Deep Descent',label:'Gyro Drop',version:'gd-descent-v2',url:'/descent/'},
 {game:'core-pins',title:'Core Pins',label:'Core Pins',version:'cp-phaser-v1',url:'/challengers/play?game=core-pins'},
 {game:'nova-merge',title:'Nova Merge',label:'Nova Merge',version:'nm-phaser-v1',url:'/challengers/play?game=nova-merge'}]);
...
function safeReturn(raw){try{const u=new URL(raw,location.origin);if(u.origin!==location.origin)return null;if(['/descent','/descent/','/descent/index.html'].includes(u.pathname))return '/descent/';if(['/arcade3/play','/arcade3/play.html','/challengers/play','/challengers/play.html'].includes(u.pathname)&&GAMES.some(g=>g.game===u.searchParams.get('game')))return u.pathname.replace('.html','')+'?game='+encodeURIComponent(u.searchParams.get('game'));return u.pathname==='/games/orbit-sprint/'||u.pathname==='/games/orbit-sprint'?u.pathname:null;}catch{return null;}}
