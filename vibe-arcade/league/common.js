/* Shared presentation only. No auth, score, rules or country writes. */
(function(){'use strict';
...
function countdown(){const now=new Date(),start=new Date(Date.UTC(now.getUTCFullYear(),now.getUTCMonth(),now.getUTCDate()));start.setUTCDate(start.getUTCDate()-((start.getUTCDay()+6)%7));const end=new Date(start.getTime()+7*86400000);let minutes=Math.max(0,Math.floor((end-now)/60000)),days=Math.floor(minutes/1440),hours=Math.floor(minutes%1440/60);minutes%=60;const text=days+'d '+String(hours).padStart(2,'0')+'h '+String(minutes).padStart(2,'0')+'m';document.querySelectorAll('[data-countdown]').forEach(n=>n.textContent=text);const fmt={month:'short',day:'numeric',timeZone:'UTC'};document.querySelectorAll('[data-week-label]').forEach(n=>n.textContent=start.toLocaleDateString('en-US',fmt)+' — '+new Date(end-1).toLocaleDateString('en-US',fmt)+' UTC');}
function identity(){const p=C?.profile,target=$('#leagueIdentity');if(target){if(p)label(target,p.country,p.handle);else target.textContent='Sign in →';}for(const a of document.querySelectorAll('[data-nav="country"]'))a.href='/community/?game=all&scope=country'+(p?.country?'&country='+encodeURIComponent(p.country):'');document.dispatchEvent(new CustomEvent('league:identity'));}
...
function track(e,props){window.LJTelemetry?.track(e,'league','league-ui-v1',props||{});}
window.WorldLeague={C,GAMES,node,number,label,readBoard,safeReturn,ready,identity,track};
countdown();setInterval(countdown,60000);
const navPath=location.pathname;document.querySelectorAll('.mobile-nav a').forEach(a=>{if(navPath.startsWith('/community')&&a.textContent.includes('Rankings')||!navPath.startsWith('/community')&&a.textContent.includes('League'))a.setAttribute('aria-current','page');});
track('page_view');
})();
