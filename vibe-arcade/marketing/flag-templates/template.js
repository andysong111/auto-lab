...
const verified=P.get('verified')==='1',cover=P.get('cover')==='1';
...
window.LoopJoltFlagTemplate=Object.freeze({format,verified,cover,flags:flagList,diagnostics:()=>({format,verified,cover,flagCount:[...document.querySelectorAll('img')].filter(x=>getComputedStyle(x).display!=='none'&&x.getClientRects().length).length,overflow:frame.scrollWidth>1080||frame.scrollHeight>1920,warning:!warning.hidden})});
})();