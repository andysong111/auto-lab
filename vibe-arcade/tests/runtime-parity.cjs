/* Stage 2-1 unchanged-experience gate. Baseline is the owner-approved released build.
...
function unchanged(){
 const paths=['descent/core.js','descent/view.js','descent/art.js','descent/audio.js','descent/style.css','community/client.js','community/flags.css','arcade3/clock.js','arcade3/telemetry.js','package.json','package-lock.json'];
 for(const p of paths)assert.equal(fs.readFileSync(path.join(root,p),'utf8'),baseline('vibe-arcade/'+p),'Protected file changed: '+p);
 const html=fs.readFileSync(path.join(root,'descent/index.html'),'utf8').replace(/<script src="\/runtime\/[^\"]+"><\/script>/g,'');
