...
for(const [name,size] of [['desktop',{width:1366,height:1000}],['mobile',{width:390,height:844}]]) {
 ...
 page=await context.newPage();page.on('pageerror',e=>errors.push(e.message));await page.goto('http://127.0.0.1:8765/community/?game=perfect-timing');await page.waitForFunction(()=>document.querySelectorAll('#rows tr').length===1);await loadedFlag(page,'#rows tr:first-child');assert(await page.locator('#login').isDisabled());assert.equal(await page.locator('#country option').count(),250);assert((await page.locator('#rows tr').first().innerText()).includes('andy'));assert((await page.locator('#rows tr').first().innerText()).includes('592'));
 ...
 await page.evaluate(()=>sessionStorage.setItem('loopjolt_google_session_v1',JSON.stringify({token:'QA-not-a-real-token',expires:Date.now()+3600000})));await page.reload();await page.waitForFunction(()=>document.querySelector('#accountTitle').textContent.includes('TestPilot'));await loadedFlag(page,'#accountTitle');await loadedFlag(page,'#countryFlagPreview');
 ...
 // Renderer rejects paths/HTML as country values; labels are text, never markup.
 ...
 // Same common client upgrades the existing Orbit and Arcade 3 header identity labels.
 ...
 // Missing images fail visibly to the correct country code, without infinite retries.
