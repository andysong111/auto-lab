...
const S=require('../scripts/build-search-metadata.cjs'),root=path.resolve(__dirname,'..');
// Stage 4 extends the original search pass; compare against the FINAL built HTML/map.
const title=file=>fs.readFileSync(path.join(root,file),'utf8').match(/<title>(.*?)<\/title>/)[1];
...
(async()=>{
 ...
 for(let i=0;i<24;i++){try{const r=await get('/');if(r.text.includes('<title>'+title('index.html')+'</title>')){ready=true;break;}}catch{}await new Promise(r=>setTimeout(r,5000));}
 assert(ready,'deployed SEO build not ready');
 for(const [file,p] of Object.entries(S.PAGES)){const route=new URL(p.url).pathname,r=await get(route);assert(r.text.includes('<title>'+title(file)+'</title>'));assert(r.text.includes('<link rel="canonical" href="'+p.url+'">'));assert.equal((r.text.match(/rel="canonical"/g)||[]).length,1);assert(r.text.includes('property="og:description"'));if(file==='index.html')assert(r.text.includes('<h2>Deep Descent</h2>'));checks.push({route,metadata:true});}
 const map=await get('/sitemap.xml');assert.equal(map.text.trim(),fs.readFileSync(path.join(root,'sitemap.xml'),'utf8').trim());
 // Retain old query-route checks; canonical new game routes are covered by acquisition-live.
