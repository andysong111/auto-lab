/* Stage 2-3 unchanged-experience gate. Baseline is the owner-approved released build.
 * This uses a MOCK public player and canonical rules; never writes production data. */
'use strict';
const assert=require('node:assert/strict'),fs=require('node:fs'),path=require('node:path'),cp=require('node:child_process');
const {chromium}=require(process.env.PW_MODULE||'playwright');
const root=path.resolve(__dirname,'..'),repo=path.resolve(root,'..'),base=process.env.FEEL_BASE_REF;
if(!base&&!process.env.BASELINE_DIR)throw Error('Provide FEEL_BASE_REF explicitly for this one-time parity comparison.');
const out=process.env.FEEL_QA_OUT||'/tmp/feel23-parity';fs.mkdirSync(out,{recursive:true});
...
function unchanged(){
 const paths=['descent/core.js','descent/art.js','descent/style.css','community/client.js','community/flags.css','arcade3/clock.js','arcade3/telemetry.js','package.json','package-lock.json','runtime/core.js','runtime/community-adapter.js','shell/core.js','descent/shell.js'];
 for(const p of paths)assert.equal(fs.readFileSync(path.join(root,p),'utf8'),baseline('vibe-arcade/'+p),'Protected file changed: '+p);
 const html=fs.readFileSync(path.join(root,'descent/index.html'),'utf8').replace(/<script src="(?:\/feel\/phaser-fx|\/feel\/audio|\/descent\/feel)\.js"><\/script>/g,'');
...
async function load(browser,width,before){
 const context=await browser.newContext({viewport:{width,height:1000},deviceScaleFactor:1,isMobile:width<600,hasTouch:width<600,reducedMotion:'no-preference'}),p=await context.newPage();p.errors=[];p.on('pageerror',e=>p.errors.push(e.message));
 ...
 for(const file of scripts){if(file.includes('community/client')||file.includes('telemetry'))continue;const data=before?baseline('vibe-arcade'+file):fs.readFileSync(path.join(root,file),'utf8');await p.addScriptTag({content:data});}
