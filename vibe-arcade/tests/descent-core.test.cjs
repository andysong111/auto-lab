...
test('shutter opens and closes on canonical motion time',()=>{const s=R.create(1);s.floor=15;const r=s.rings[15];s.motion=R.mod(20-r.phase,360);assert.equal(R.ringAt(s).open,false);s.rotation=R.mod(R.CONTACT-r.gap-Math.trunc(s.motion*r.speed/2));assert.equal(R.contact(s).kind,'gate');s.motion=R.mod(160-r.phase,360);assert.equal(R.ringAt(s).open,true);});
