...
async function execute(config){
 ...
 async function session(width,height,seed,{practice=false,reducedMotion='no-preference',video=false}={}){
  ...
  await installGuard(ctx,server.origin,effects);await ctx.addInitScript(canvasAudit);await ctx.addInitScript(terminalTimeline,{statePath:contract.completion.state_path});
