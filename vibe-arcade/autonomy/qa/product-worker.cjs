...
const {canvasAudit,geometry,collisions}=require('./product-browser.cjs');
...
async function execute(config){
 ...
 async function visible(p,selector,{initial=true}={}){
  ...
  const b=await l.boundingBox(),vp=p.viewportSize();expect(!initial||(b.x>=0&&b.y>=0&&b.x+b.width<=vp.width+1&&b.y+b.height<=vp.height+1),'inside initial viewport',{selector,box:b,viewport:vp});return (await l.innerText()).trim();
 }
 ...
 async function feedback(p,ctx,mode,reduced){
  ...
  const before=await snap(p),initial=digest(await p.locator(contract.feedback.visual_probe).screenshot());
  ...
  const frames=[];for(const ms of [0,60,contract.feedback.settle_ms]){if(ms)await p.clock.runFor(ms);const s=await snap(p);frames.push({ms,visual:digest(await p.locator(contract.feedback.visual_probe).screenshot()),active:at(s,contract.feedback.active_probe_path),static:at(s,contract.feedback.static_probe_path)});}
  await capture(p,'feedback-'+(reduced?'reduced':'motion'));
 ...
 async function runPlan(p,ctx,seed,mode,{captureMilestones=false}={}){
  ...
  const traces=[],stages=[],milestones=new Set(),perStage=new Map();
  for(const step of plan){
   s=await snap(p);const n=g.nodes[step.from];
   if(!perStage.has(n.stage)){const metrics=stageMetrics(g,step.from);expect(at(s,contract.difficulty.stage_path)===metrics.stage,'stage cross-check',{metric:at(s,contract.difficulty.stage_path),oracle:metrics.stage});expect(at(s,contract.difficulty.complexity_path)===metrics.complexity,'complexity agrees with reviewed consequential transition width',{declared:at(s,contract.difficulty.complexity_path),derived:metrics.complexity});perStage.set(n.stage,{...metrics,actual_actions:0});}
   const before=s,after=await input(p,ctx,step.action,mode);id=identify(model,g,after);expect(id===step.to,'reviewed normal-input transition '+step.to,{actual:id,projection:model.projection.map(k=>at(after,k))});
