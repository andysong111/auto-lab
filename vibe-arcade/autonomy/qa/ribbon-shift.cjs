 ...
 try{
  ...
  for(let n=0;n<seeds.length;n++){
   ...
   try{
    ...
    await check('product_reduced_motion',async()=>{for(const reduced of [true,false]){await p.emulateMedia({reducedMotion:reduced?'reduce':'no-preference'});await p.clock.runFor(100);assert.equal(await p.locator('html').getAttribute('data-reduced-motion'),String(reduced));const view=await p.evaluate(()=>globalThis.RibbonPresentation?.snapshot());assert.equal(view?.reducedMotion,reduced);assert.equal(view.activeVoices,0);assert(view.particles>=0&&view.particles<=32);if(reduced){assert.equal(view.particles,0);assert.equal(await p.locator('*').evaluateAll(ns=>ns.filter(n=>{const s=getComputedStyle(n);return s.animationName!=='none'&&parseFloat(s.animationDuration)>.01;}).length),0);}}});
