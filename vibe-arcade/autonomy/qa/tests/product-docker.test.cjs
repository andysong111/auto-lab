...
for(const [caseIndex,[variant,expected]] of Object.entries(cases).entries())if(caseIndex%shards===shard)test('Docker Chromium product fixture '+variant,{timeout:200000},async t=>{
 ...
 if(variant==='MULTI_BAD'){
  ...
  const compactCodes=[...new Set(prompt.previous_failures.filter(f=>f.code!=='additional_failure_instances_compacted').map(f=>f.code))].sort(),requestCodes=[...new Set(request.qa_failures.map(f=>f.code))].sort();assert.deepEqual(compactCodes,requestCodes);assert(prompt.previous_failures.some(f=>f.code==='additional_failure_instances_compacted'));assert(prompt.qa_evidence.product_qa.checks.length>=expected.length);
