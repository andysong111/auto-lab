...
if(require.main===module){const dir=path.join(__dirname,'../../reviewed'),m=model();atomicJSON(path.join(dir,'fixture-switches-v1.json'),m);atomicJSON(path.join(dir,'registry.json'),{schema_version:1,entries:[{id:'fixture-switches-v1',file:'fixture-switches-v1.json',sha256:hash(m),scope:'fixture',reviewed_by:'Factory infrastructure fixture review',rationale:'Toggle truth table; target has 2, 3, 4 set bits; 3, 4, 5 consequential choices. Test-only approval cannot commission a public candidate.'}]});atomicJSON(path.join(__dirname,'contract.json'),contract());}
