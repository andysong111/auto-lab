...
const core={
 create(seed){return {seed,stage:1,completed:0,mask:0,target:target(seed,1),outcome:'playing',tick:0,score:0,actions:0,interactions:0,pointerActions:0,latch:false};},
 step(s,i){s.tick++;if(s.outcome!=='playing')return;
 const held=!!(i.x||i.y||i.action||i.pointer);
 const bit=i.pointer?Math.floor(i.pointer.x*5):i.x<0?0:i.x>0?1:i.y<0?2:i.y>0?3:i.action?4:-1;
 const signature=held?(i.pointer?'p':'k')+bit:false;
 if(held&&signature!==s.latch){if(i.pointer)s.pointerActions++;
 ...
 s.latch=signature;if(s.tick>=900&&s.outcome==='playing')s.outcome='failure';
 },
