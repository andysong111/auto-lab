...
function canvasAudit(){
 ...
 for(const method of ['fillText','strokeText']){const original=proto[method];proto[method]=function(text,x,y,maxWidth){
  ...
  const points=[[left,top],[left+width,top],[left,top+font],[left+width,top+font]].map(([a,c])=>({x:b.x+(t.a*a+t.c*c+t.e)*sx,y:b.y+(t.b*a+t.d*c+t.f)*sy}));
  const row={text:String(text).slice(0,200),x:Math.min(...points.map(p=>p.x)),y:Math.min(...points.map(p=>p.y)),width:Math.max(...points.map(p=>p.x))-Math.min(...points.map(p=>p.x)),height:Math.max(...points.map(p=>p.y))-Math.min(...points.map(p=>p.y)),font_px:font*Math.hypot(t.c,t.d)*sy};
