...
function compactRepairRequest(request){
  ...
  const out={};
  for(const k of ['schema_version','game_id','version','target_version','attempt','max_repair_attempts','source_hash','repair_stage','deferred_failure_counts','product_evidence_index','product_acceptance','commercial_evidence_index','commercial_acceptance','operation_id'])if(request[k]!==undefined)out[k]=request[k];
  return out;
}
