...
const contract=Object.freeze({
  version:'gamekit-2-phaser4',factory_supplied:['phaser.js','phaserkit.js','gamekit.js','manifest.json'],
  files:{'core.js':'DOM-free create(seed), step(state,input), observe(state), terminal(state). Export with globalThis.GameCore = {create,step,observe,terminal}; NEVER use window even for export. Core source is statically checked for document/window/localStorage/fetch/Phaser identifiers. Mutate only supplied state; no time/random globals.',
    'app.js':"Fetch ./manifest.json, create renderer=PlayJoltPhaserKit.create({canvas,art:GameArt,metadata}), then call PlayJoltGameKit.create({core:GameCore,renderer,canvas,metadata,presentation:renderer.presentation}). No private game loop, direct input listeners or replacement diagnostics.",
    ...
    'index.html':'Load ./phaser.js, ./phaserkit.js, ./core.js, ./view/art.js, ./gamekit.js, ./app.js in that order; local ./style.css. Include noindex,nofollow and viewport meta.',
...
No copyrighted character, art or code copying. Build original presentation with Phaser 4 scene objects, Graphics, sprites, particles, filters, tweens and cameras using the factory-supplied local runtime or explicitly approved local assets only. Keep all readable/decision-critical text in audited DOM elements; do not use Phaser Text for game labels. No CDN, package install, network service, external runtime dependency, account, analytics, score submission or login.
...
Repair responses replace only the allowed files and preserve unrelated game behavior. Obey repair_request.repair_stage: fix the active layer first and do not churn already-passing layers. If repair_strategy is structural_rewrite, you may reorganize any allowed candidate files to remove the root cause instead of repeating local patches. Never edit manifest.json, phaser.js, phaserkit.js, gamekit.js, quality policy, diagnostics probes or acceptance tests.`;
