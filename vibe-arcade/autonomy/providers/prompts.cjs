...
const contract=Object.freeze({
  ...
  lifecycle:'GameKit owns boot/start/pause/resume/finish/restart, responsive canvas, visibility/pagehide, error boundary, local practice best, contract-bound objective/progress/result/best/replay text, named keyboard/touch action mapping, reduced-motion state and GameDiagnostics.snapshot(). Do not override GameDiagnostics or GameKit.',
  limits:policy.limits
});
const phaserContract=Object.freeze({
  ...contract,
  version:'gamekit-2-phaser4',
  factory_supplied:['gamekit.js','phaser-runtime.js','phaser-gamekit.js','manifest.json'],
  files:{...contract.files,
    'app.js':"Fetch ./manifest.json, then PlayJoltPhaserKit.create({core,sceneHooks:PlayJoltPhaserScene,canvas:document.querySelector('[data-game-canvas]'),metadata,presentation}). No private simulation loop and no direct Phaser input for gameplay.",
    'view/art.js':'Export globalThis.PlayJoltPhaserScene = {create(scene,runtime), render(scene,snapshot,size), resize?(scene,size), destroy?(scene)}. Phaser is presentation only. Use snapshot clones; never mutate canonical state or own gameplay input.',
    'index.html':'Load ./phaser-runtime.js, ./core.js, ./view/art.js, ./gamekit.js, ./phaser-gamekit.js, ./app.js in that order; local ./style.css. Include noindex,nofollow and viewport meta.'},
  lifecycle:'GameKit owns canonical simulation/input/lifecycle; Phaser owns presentation only. Phaser input, physics callbacks and timers must never directly mutate gameplay state. Use GameKit named actions and snapshots.'
});
...
Use the supplied GameKit API exactly. If immutable_spec.render_runtime is phaser4, use PlayJoltPhaserKit and the factory-supplied local Phaser runtime; Phaser is visual-only and canonical gameplay remains deterministic in core.js. Never load Phaser from a CDN or emit phaser-runtime.js/phaser-gamekit.js yourself. Return only the structured file result; tests are suggestions, never claims of executed tests.
...
Repair responses replace only the allowed files and preserve unrelated game behavior. Obey repair_request.repair_stage: fix the active layer first and do not churn already-passing layers. If repair_strategy is structural_rewrite, you may reorganize any allowed candidate files to remove the root cause instead of repeating local patches. Never edit manifest.json, gamekit.js, quality policy, diagnostics probes or acceptance tests.`;
...
function compile({root,workspace,manifest,spec,request,operationId,budget,rejected=null}) {
  ...
  return {operation_id:operationId,game_id:manifest.game_id,version:manifest.version,mode,immutable_spec:spec,
    ...
    gamekit_contract:{...(manifest.render_runtime==='phaser4'?phaserContract:contract),source_hash:hash(fs.readFileSync(path.join(__dirname,'../gamekit/gamekit.js'),'utf8'))},
...
module.exports={compile,instructions,contract,phaserContract};
