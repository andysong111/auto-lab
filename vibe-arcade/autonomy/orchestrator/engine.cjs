      ...
      for(let step=0;step<80;step++) {
        ...
        switch(m.state) {
          ...
          case 'QA_RUNNING': {
            ...
            if(fs.existsSync(file)) { const prior=readJSON(file);if(prior.source_hash===m.source_hash&&prior.policy_hash===hash(this.policy)&&prior.version===m.version&&prior.game_id===id&&(!m.product_contract||prior.contract_hash===hash(m.product_contract)&&prior.product_qa?.runner_version==='product-quality-1')&&(!m.commercial_contract||prior.commercial_contract_hash===hash(m.commercial_contract)&&prior.commercial_qa?.runner_version==='commercial-polish-1'))result=prior; }
            ...
            m=this.move(m,(result.passed||result.technical_qa?.passed&&result.product_qa)?'QUALITY_GATE':'QA_FAILED',{qa_status:result.passed?'PASS':'FAIL',technical_qa_status:(result.technical_qa||result).passed?'PASS':'FAIL',product_qa_status:result.product_qa?(result.product_qa.passed?'PASS':'FAIL'):'UNVERIFIED',commercial_qa_status:result.commercial_qa?(result.commercial_qa.passed?'PASS':'FAIL'):'UNVERIFIED',policy_hash:hash(this.policy),failure_reasons:result.hard_failures});
          ...
          case 'REPAIR_PENDING': {
            ...
            m=this.move(m,'REPAIRING',{repair_attempt:request.attempt,version:request.target_version,source_path:`autonomy/games/${id}/${request.target_version}`,qa_status:'PENDING',technical_qa_status:'PENDING',product_qa_status:'PENDING',commercial_qa_status:'PENDING',quality_status:'PENDING',release_status:'NONE',preview_url:null});break;
          }
          case 'REPAIRING': {
