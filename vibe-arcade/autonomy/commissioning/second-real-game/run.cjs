'use strict';
// One trusted QA composition; existing Worker/Factory still own every transition, call and repair.
const fs=require('node:fs'),path=require('node:path');
const {AutonomousWorker}=require('../../worker/runtime.cjs'),{DockerQA}=require('../../isolation/docker-qa.cjs');
const {atomicJSON,readJSON,hash}=require('../../orchestrator/files.cjs');
class ProductQA {
 ...
 async run(c){
  ...
  // Independent product QA collects early UI/motion defects even when reachability fails.
  const product=await new DockerQA({limits:this.limits,signal:this.signal}).run({...c,outDir:path.join(c.outDir,'product'),product:'cloud-cargo',policy:{...c.policy,limits:{...c.policy.limits,run_timeout_ms:300000}}});
  const merged={...common,passed:common.passed&&product.passed,hard_failures:[...common.hard_failures,...product.hard_failures],console_errors:[...common.console_errors,...product.console_errors],page_errors:[...common.page_errors,...product.page_errors],side_effects:[...common.side_effects,...product.side_effects],screenshots:[...common.screenshots.map(x=>'common/'+x),...product.screenshots.map(x=>'product/'+x)],duration_ms:common.duration_ms+product.duration_ms,product_evidence:{passed:product.passed,report:'product/qa.json',seeds:product.browser_cases.map(x=>x.seed),source_hash:product.source_hash},policy_hash:hash(c.policy)};
  atomicJSON(path.join(c.outDir,'qa.json'),merged);return merged;
 }
}
async function run(){const root=process.env.FACTORY_WORKER_ROOT,settings=readJSON(path.join(root,'worker-config.json')),controller=new AbortController();
 for(const name of ['SIGINT','SIGTERM'])process.once(name,()=>controller.abort(Error('worker stopped')));
 const worker=new AutonomousWorker({root,settings,policyFile:path.join(root,'control-policy.json'),qa:new ProductQA({limits:settings.isolation,signal:controller.signal}),signal:controller.signal});
