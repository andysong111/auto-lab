...
const summary={schema:'playjolt-fourth-real-game/1',game_id:id,model:'gpt-5.6-terra',run_id:process.env.GITHUB_RUN_ID,runner_commit:process.env.GITHUB_SHA,operations,total_calls:operations.length,total_estimated_cost:Number(cumulative.toFixed(8)),billed_cost:null,model_source:'real OpenAI provider, no injected mock',factory:m?{state:m.state,version:m.version,repair_attempt:m.repair_attempt,qa_status:m.qa_status,technical_qa:m.technical_qa,product_qa:m.product_qa,quality_gate:m.quality_gate,quality_status:m.quality_status,source_hash:m.source_hash,failure_reasons:m.failure_reasons}:null,production_authorized:false};
