...
const oracle=review(proposal.product_contract);
...
Object.assign(policy,{mode:'fourth-real-game-one-candidate',intake_enabled:true,auto_rc_enabled:false,max_active_jobs:1,max_daily_new_jobs:1});
Object.assign(policy.kill_switches,{release_candidates:true,production_shipping:true,marketing_promotion:true});
for(const [name,data] of Object.entries({'spec-gate.json':gate,'product-preflight.json':{schema:'PASS',review:'PASS',approval:oracle.approval},'factory-spec.json':gate.factory_spec,'worker-config.json':settings,'control-policy.json':policy,'authorization.json':approval}))atomicJSON(path.join(root,name),data);
