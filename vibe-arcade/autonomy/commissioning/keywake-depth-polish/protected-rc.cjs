...
async function main(){
 ...
 while(Date.now()-started<15*60000){
  const r=await fetch(`https://api.github.com/repos/${repo}/contents/vibe-arcade/autonomy/commissioning/keywake-depth-polish/rc-sealed-payload.json?ref=commissioning%2Fkeywake-depth-polish-20260923`,{headers:{Authorization:'Bearer '+process.env.GITHUB_TOKEN,Accept:'application/vnd.github+json'},signal:AbortSignal.timeout(10000)});
