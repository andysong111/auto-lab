...
function heightAt(code,index){return Math.floor(code/pow3(index))%3;}
function setHeight(code,index,value){const p=pow3(index),old=heightAt(code,index);return code+(value-old)*p;}
function targetFor(pack,stage){
  const n=activeCount(stage);let code=0;
  for(let i=0;i<n;i++)code+= (1+((pack+i+stage)%2))*pow3(i);
  return code;
}
const id=v=>v.join(':');
function transitions(v,pack){
  const [stage,cursor,code,outcome]=v;if(outcome!=='playing')return [];
  const n=activeCount(stage),edges=[];
  edges.push({action:'left',values:[stage,(cursor+n-1)%n,code,outcome]});
  edges.push({action:'right',values:[stage,(cursor+1)%n,code,outcome]});
  edges.push({action:'up',values:[stage,cursor,setHeight(code,cursor,(heightAt(code,cursor)+1)%3),outcome]});
  edges.push({action:'down',values:[stage,cursor,setHeight(code,cursor,(heightAt(code,cursor)+2)%3),outcome]});
  if(code===targetFor(pack,stage)){
    if(stage<3)edges.push({action:'lock',values:[stage+1,0,0,'playing']});
...
function oracle(){
  const model={schema_version:1,projection:['state.stage','state.cursor','state.code','state.outcome'],seeds:{}};
...
function contracts(model){
  ...
  const commercialContract={
    ...
    visual_legibility:{pairs:[{id:'active-future-beacon',kind:'action_availability',a:anchor('1:0:0:playing',region(.04,.29,.19,.31)),b:anchor('1:0:0:playing',region(.77,.29,.19,.31)),action_a:'up',action_b:'lock',marker:'An active beacon is a solid luminous pylon with a target ghost and base ring; a future beacon is an empty recessed plinth with a broken outline.'}]},
    state_distinction:{pairs:[{id:'low-raised-beacon',kind:'state',a:anchor('1:0:0:playing',region(.04,.23,.22,.39)),b:anchor('1:0:1:playing',region(.04,.23,.22,.39)),state_path:'state.code',marker:'Raising a beacon visibly changes its physical height, bright core area, cap position and vertical energy column, not only text or color.'}]},
    action_feedback:{probes:[
      {id:'raise-beacon',node:'1:0:0:playing',action:'up',kind:'movement',region:region(.02,.20,.25,.45)},
      {id:'move-selector',node:'1:0:0:playing',action:'right',kind:'movement',region:region(.02,.42,.50,.22)},
      {id:'forge-tier',node:'1:0:5:playing',action:'lock',kind:'unlock',region:region(0,.03,1,.43)}
    ]},
    motion:{intermediate_ms:[70,150],settle_ms:450},
    progression_spectacle:{checkpoints:[anchor('1:0:0:playing',region(0,.03,1,.48)),anchor('2:0:0:playing',region(0,.03,1,.48)),anchor('3:0:0:playing',region(0,.03,1,.48))],marker:'The forge expands from two to three to four active pylons; each completed tier leaves a persistent molten ring, enlarges the central crown core and adds denser overhead geometry.'},
...
function ruleData(){return {
  ...
  stage_widths:[2,3,4],
  heights:[0,1,2],
  transform_rule:'LEFT and RIGHT wrap the selected active beacon. UP raises only the selected beacon 0->1->2->0. DOWN lowers only the selected beacon 0->2->1->0. UP then DOWN and LEFT then RIGHT return to the identical reviewed state. LOCK changes reviewed state only when every active beacon exactly matches the seeded target heights.',
  target_rule:'Every active beacon target is height 1 or 2, alternating by seed parity, beacon index and stage. A correct LOCK resets cursor and heights, increments completed and advances the stage; stage3 LOCK succeeds.'
};}
function proposal(model,productContract,commercialContract){
  return {
    ...
    qa:{seed:12,keyboard:{key:'ArrowUp',observation:'state.code'},pointer:{observation:'state.code'},terminal_ms:52000},
    implementation_contract:{
      ...
      progression:'Canonical state starts stage=1, cursor=0, code=0, outcome="playing", completed=0. Stage widths are 2,3,4 beacons. LEFT/RIGHT wrap selection across only active beacons. UP/DOWN modify only the selected base-3 height digit and are exact opposites. Blocked FORGE does not change the reviewed projection. Correct FORGE resets cursor/code, increments completed and advances stage; stage3 FORGE immediately sets stage=4 and outcome="success". meaningful_actions increments for every reviewed projection-changing edge including successful FORGE, never for blocked FORGE.',
      presentation:'Use Phaser 4 Game Objects, Graphics, Tweens, Particles and Camera effects only through factory-owned PlayJoltPhaserKit. Build an original dark foundry with four large pylon plinths, physical towers at three discrete heights, a bright selector ring, translucent target-height ghosts, a central crown core and persistent molten tier rings. Stage1 activates two pylons, stage2 three, stage3 four. Height changes must move large geometry vertically; selection changes must move a large halo. Do not use candidate Canvas 2D drawing, create Phaser.Game, create a custom RAF loop or make Phaser physics authoritative.',
      originality:'Beacon Forge is seeded height tuning with a movable selector and reversible three-level pylons. It is not pulse weaving, coupled gears, resonance sigils, row/column cycling, cargo assignment, optical routing, auto-running, survival, merge, reaction or single-button timing.',
      difficulty:'Implement the reviewed rule data exactly: '+JSON.stringify(ruleData())+' Every stage target gives each active pylon a nonzero target, so an efficient solution requires at least one height decision per pylon plus navigation and FORGE. quality.stage=state.stage. quality.complexity=active beacon count. quality.objective_progress=completed. quality.meaningful_actions increments once for each reviewed projection-changing edge. quality.reversible_state_key is stage:cursor:code:outcome.',
...
function installReviewData(model,productContract,commercialContract){
  ...
  appendRegistry(path.join(ROOT,'qa/reviewed/registry.json'),{id:'beacon-forge-v1',file:'beacon-forge-v1.json',sha256:hash(model),scope:'candidate',game_id:GAME_ID,contract_sha256:hash(productContract),reviewed_by:'ChatGPT trusted pre-generation design review 2026-09-25',rationale:'Finite base-3 beacon graph across six deterministic seeds. Stage widths are 2/3/4, every target pylon is nonzero, cursor and height actions have explicit inverses, all reachable reviewed states remain bounded, and the three stages are solvable without hidden timing.'});
  appendRegistry(path.join(ROOT,'qa/commercial/reviewed/registry.json'),{id:'beacon-forge-commercial-v1',scope:'candidate',contract_sha256:hash(commercialContract),product_contract_sha256:hash(productContract),reviewed_by:'ChatGPT trusted pre-generation commercial review 2026-09-25',rationale:'Reviewed large pylon height distinction, selector travel, tier forge transition, 2/3/4-pylon progression, explicit crown result transformation, intentional silence, mobile hierarchy and bounded performance using normal-player regions only.'});
