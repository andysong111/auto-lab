...
async function run(){
 ...
 const qa={assertAvailable:()=>docker.assertAvailable(),run:async c=>{
  ...
  console.log(JSON.stringify({qa_end:c.manifest.version,technical:result.technical_qa?.passed,product:result.product_qa?.passed,commercial:result.commercial_qa?.passed,failures:result.hard_failures.map(f=>({code:f.code,check:f.check,selector:f.selector,state_path:f.state_path,expected:f.expected,actual:f.actual,evidence:f.evidence}))}));
  return result;
 }};
