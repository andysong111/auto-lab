...
function auditedProvider(provider,root){return {identity:provider.identity,assertAvailable:()=>{checkFeedback(root);return provider.assertAvailable();},cancel:id=>provider.cancel(id),generate:async args=>{
 ...
 console.log(JSON.stringify({provider_start:req.operation_id,failures:req.previous_failures.map(x=>({code:x.code,check:x.check})),validation:req.model_validation}));
 const result=await provider.generate(args);console.log(JSON.stringify({provider_return:req.operation_id,usage:result.usage}));return result;
}};}
