...
if(process.argv[2]==='probe') {require('./probe.cjs');}
  ...
  if(!['technical','product','commercial'].includes(config.suite||'technical'))throw Error('invalid QA suite');
  const worker=config.suite==='commercial'?'commercial-worker.cjs':config.suite==='product'?'product-worker.cjs':'worker.cjs';
  const child=spawn(process.execPath,['/opt/factory/qa/'+worker,'/input/qa.json'],{env:{PATH:'/usr/bin:/bin',HOME:'/tmp',PLAYWRIGHT_BROWSERS_PATH:'/ms-playwright'},stdio:'inherit'});
