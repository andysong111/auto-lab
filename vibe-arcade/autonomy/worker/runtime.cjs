...
class AutonomousWorker {
  ...
  constructor({root=APP,policyFile=DEFAULT_POLICY,settings={},provider=null,qa=null,release=null,signal,now=Date.now}={}) {
    this.root=path.resolve(root);this.policyFile=path.resolve(policyFile);this.settings=settings;this.limits=config(settings);this.provider=provider;this.qa=qa;this.release=release;this.signal=signal;this.now=now;
  ...
  async runOnce() {
    ...
    try{return await this.locks.withLock('GAME-00000000-000',async lockSignal=>{
      ...
      try {
        ...
        const prices=pricing(this.settings.pricing,{mock:!!this.provider,now:this.now()});
